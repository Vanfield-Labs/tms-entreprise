...
export default function FuelRequests() {
  ...
  const { toastMessage, showToast } = usePageToast();
  ...
  const load = async () => {
    ...
    const vehicleIds = [...new Set(raw.map((r) => r.vehicle_id).filter(Boolean))] as string[];
    ...
    const dMap = Object.fromEntries(
      (drivers ?? []).map((d: any) => [d.id, d.full_name || d.license_number])
    );
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "fuel_requests" },
        async (payload) => {
          const nextStatus =
          ...
          if (payload.eventType === "INSERT") {
            showToast("Fuel request submitted");
            if (requestId) flash(requestId);
              ...
              showToast(
                `Fuel request ${
                  nextStatus === "submitted"
                    ? "submitted"
                    : `status updated to ${STATUS_LABEL[nextStatus as FuelStatus] ?? nextStatus}`
                }`
              );
            } else {
              showToast("Fuel request updated");
            }

            if (requestId) flash(requestId);
          } else if (payload.eventType === "DELETE") {
            showToast("Fuel request removed");
          }
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <CardListSkeleton count={4} showHeaderMeta showExpandedPreview />
      ) : filtered.length === 0 ? (
