...
type FuelRow = {
  ...
  liters: number | null; amount: number | null; mileage: number | null;
  receipt_url: string | null; request_date: string; created_at: string;
  vehicles: { plate_number: string; fuel_type: string | null; current_mileage: number | null } | null;
  profiles: { full_name: string } | null;
};

type RecordState = {
  liters: string; amount: string; mileage: string; vendor: string;
  notes: string; receiptFile: File | null; uploading: boolean; saving: boolean; error: string;
};
const EMPTY_STATE: RecordState = { liters:"",amount:"",mileage:"",vendor:"",notes:"",receiptFile:null,uploading:false,saving:false,error:"" };
...
export default function FuelRecordQueue() {
  ...
  const u = (id:string,patch:Partial<RecordState>) => setState(m=>({...m,[id]:{...s(id),...patch}}));

  const load=async()=>{
    setLoading(true);
    const {data,error}=await supabase.from("fuel_requests")
  ...
  const record=async(id:string)=>{
    ...
    try {
      ...
      const {error}=await supabase.rpc("record_fuel_request",{
        p_fuel_request_id:id, p_actual_liters:parseFloat(st.liters),
        p_actual_amount:parseFloat(st.amount), p_vendor:st.vendor.trim()||null,
        p_mileage:st.mileage?parseFloat(st.mileage):null,
        p_receipt_url:receiptUrl, p_notes:st.notes.trim()||null,
      });
      if(error) throw error;
      setState(m=>{const n={...m};delete n[id];return n;});
      await load();
    } catch(e:any){u(id,{error:e.message??"Failed to record."});}
    finally{u(id,{saving:false,uploading:false});}
  };
    ...
    <div className="space-y-4">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm" style={{color:"var(--text-muted)"}}>Enter dispensed amount for approved fuel requests</p>
        </div>
      </div>
          ...
          <div className="space-y-4">
            {pg.slice.map(r=>{
              ...
              return (
                <Card key={r.id}>
                  {/* Header */}
                  <div className="px-4 py-3 border-b" style={{borderColor:"var(--border)",background:"var(--green-dim)"}}>
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="font-bold text-sm" style={{color:"var(--text)"}}>{r.profiles?.full_name??"—"}</p>
                        <p className="text-xs mt-0.5" style={{color:"var(--text-muted)"}}>
                          {r.vehicles?.plate_number??"—"} · <span className="capitalize">{r.vehicles?.fuel_type??"—"}</span> · {fmtDate(r.request_date)}
                        </p>
                        {r.purpose&&<p className="text-xs mt-1 italic" style={{color:"var(--text-muted)"}}>"{r.purpose}"</p>}
                      </div>
                      <Badge status={r.status}/>
                    </div>
                  </div>

                  {/* Input fields — 2 col grid, never overflows */}
                  <div className="p-4 space-y-3">
                    {st.error&&<p className="text-xs px-3 py-2 rounded-lg" style={{background:"var(--red-dim)",color:"var(--red)"}}>{st.error}</p>}

                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Liters Dispensed *">
                        <Input type="number" min="0" step="0.1" placeholder="0.0" value={st.liters} onChange={e=>u(r.id,{liters:e.target.value})}/>
                      </Field>
                      <Field label="Actual Cost (GHS) *">
                        <Input type="number" min="0" step="0.01" placeholder="0.00" value={st.amount} onChange={e=>u(r.id,{amount:e.target.value})}/>
                      </Field>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Mileage (km)">
                        <Input type="number" min="0" step="1" placeholder={r.vehicles?.current_mileage?.toString()??"Current km"} value={st.mileage} onChange={e=>u(r.id,{mileage:e.target.value})}/>
                      </Field>
                      <Field label="Vendor / Station">
                        <Input placeholder="e.g. Total Spintex" value={st.vendor} onChange={e=>u(r.id,{vendor:e.target.value})}/>
                      </Field>
                    </div>
                    <Field label="Notes">
                      <Input placeholder="Pump #, attendant name…" value={st.notes} onChange={e=>u(r.id,{notes:e.target.value})}/>
                    </Field>

                    {/* Receipt upload */}
                    <Field label="Receipt (optional)">
                      <div className="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer border"
                      ...
                      {st.receiptFile&&(
                        <button className="text-xs mt-1" style={{color:"var(--red)"}} onClick={()=>u(r.id,{receiptFile:null})}>Remove</button>
                      )}
                    </Field>

                    <Btn variant="primary" className="w-full" loading={st.saving||st.uploading} onClick={()=>record(r.id)}>
