...
export default function IncidentReportForm({ onSubmitted }: { onSubmitted?: () => void }) {
  const { user } = useAuth();
  const draftKey = useMemo(() => `incident_draft:${user?.id ?? "anon"}`, [user?.id]);
  ...
  useEffect(() => {
    cachedFetch<Vehicle[]>("vehicles_active_min", async () => {
  ...
  useEffect(() => {
    try {
      ...
      setIncidentType(draft.incidentType ?? "breakdown");
  ...
  }, [draftKey, vehicleId, incidentType, priority, title, description, attachments]);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm mt-1 mb-6" style={{ color: "var(--text-muted)" }}>
          Your report has been submitted. Transport supervisor will review.
        </p>
  ...
  return (
    <Card>
      <CardHeader
        ...
        subtitle="Accidents, breakdowns, maintenance or other matters"
      />
      <CardBody className="space-y-4">
        ...
        <div className="flex justify-end pt-2">
          <Btn variant="primary" onClick={submit} loading={saving || uploading}>
            Submit Report
          </Btn>
        </div>
      </CardBody>
