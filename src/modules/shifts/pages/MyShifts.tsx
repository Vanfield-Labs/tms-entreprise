...
import { PageSpinner, Card } from "@/components/TmsUI";
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const data = await cachedFetch<Record<string, ShiftEntry>>(
          `driver_schedule:${driverId}`,
          async () => {
            ...
            const { data } = await supabase
              ...
              .select(
                "shift_date,shift_code,is_override,team_name,cell_label,department_name,evening_route_name"
              )
        ...
        setSchedule(data);
        localStorage.setItem(`driver_schedule_cache:${driverId}`, JSON.stringify(data));
      } catch {
        try {
          const { data: authData } = await supabase.auth.getUser();
          const user = authData.user;
          if (user) {
            ...
            const driverId = (dr as any)?.id;
            const cached = driverId
              ? localStorage.getItem(`driver_schedule_cache:${driverId}`)
              : null;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p
                  ...
                  style={{
                    color: SHIFT_STYLES[todayEntry.shift_code]?.color ?? "var(--text)",
                  }}
                >
            ...
            return (
              ...
              >
                ...
                {entry ? (
                  ...
                  <span className="text-xs" style={{ color: "var(--text-dim)", fontSize: 9 }}>
                    —
                  </span>
                )}
              </div>
        ...
        {Object.entries(SHIFT_STYLES).map(([code, s]) => (
          <div key={code} className="flex items-center gap-1.5">
            <div
              ...
              style={{ background: s.bg, border: `1px solid ${s.color}` }}
            />
            ...
            return (
              ...
              >
                <div className="w-8 text-center">
                  <p
                    className="text-xs font-bold"
                    style={{ color: ds === today ? "var(--accent)" : "var(--text)" }}
                  >
                    {DAYS[d.getDay()]}
                  </p>
                  <p className="text-xs" style={{ color: "var(--text-dim)" }}>
