...
import {
  PageSpinner, EmptyState, Card, CardHeader, SearchInput,
...
export default function DivisionManagement() {
  ...
  const load = async () => {
    ...
    setUnits(unitList.map(u => ({ ...u, division_name: divList.find(d => d.id === u.division_id)?.name ?? "—" })));
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <TabBar tabs={TABS} active={tab} onChange={t => { setTab(t); setQ(""); }} counts={{ divisions: divisions.length, units: units.length }} />
              ...
              <Card key={d.id}>
                <div className="p-4 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[color:var(--text-muted)]">{d.unit_count} unit{d.unit_count !== 1 ? "s" : ""} · Created {fmtDate(d.created_at)}</p>
                  </div>
                ...
                <Card key={u.id}>
                  <div className="p-4 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[color:var(--text-muted)]">{u.division_name}{u.parent_unit_id ? " · Sub-unit" : ""}</p>
                    </div>
                  ...
                  <tbody>
                    {filteredUnits.map(u => (
                      <tr key={u.id}>
                        ...
                        <td className="text-[color:var(--text-muted)]">{u.parent_unit_id ? units.find(x => x.id === u.parent_unit_id)?.name ?? "—" : "—"}</td>
                        <td className="text-[color:var(--text-muted)]">{fmtDate(u.created_at)}</td>
                        <td><Btn variant="ghost" size="sm" onClick={() => openEditUnit(u)}>Edit</Btn></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
