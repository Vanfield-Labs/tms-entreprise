...
// KEY FIX: While loading, render children (fail-open).
// Never show "Feature Not Available" until licence is confirmed loaded.
...
function ExpiredBlock({ validUntil, isAdmin }: { validUntil: string | null; isAdmin: boolean }) {
  return (
    ...
    >
      ...
      {isAdmin && (
        <a href="mailto:support@tmsportal.com?subject=TMS Licence Renewal" className="btn btn-danger">
...
export function LicenceGate({ feature, children }: LicenceGateProps) {
  ...
  // FAIL-OPEN: while loading, always show children — never block
  ...
  // Feature not in this licence (only after confirmed load)
  if (!hasFeature(feature)) {
    return <LockedFeature feature={feature} />;
  }

  // Grace period — show warning banner above content
